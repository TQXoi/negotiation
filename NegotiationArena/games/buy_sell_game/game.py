...
class BuySellAgentMessage(AgentMessage):
    """Expose public actions using the canonical opponent-facing tag schema.

    The generic ``AgentMessage`` helper serializes ``key`` and ``value`` in the
    opposite order.  Besides producing malformed XML-like tags, that prevented
    the belief adapter from observing any Buy--Sell offer or acceptance.
    """
    ...
    def message_to_other_player(self):
        ...
        return (
            f"<{OTHER_PLAYER_MESSAGE}> {message} </{OTHER_PLAYER_MESSAGE}>\n"
            f"<{OTHER_PLAYER_ANSWER}> {answer} </{OTHER_PLAYER_ANSWER}>\n"
            f"<{OTHER_PLAYER_PROPOSED_TRADE}> {trade} </{OTHER_PLAYER_PROPOSED_TRADE}>\n"
        )
